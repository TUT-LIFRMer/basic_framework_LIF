...
// PowerMeter_Init_Config_s结构体
typedef struct {
    IIC_Init_Config_s iic_init_config;  // IIC配置
    uint16_t dev_address;               // 设备地址（7位地址左移1位后的值）
    const char *id;                     // 设备ID
} PowerMeter_Init_Config_s;

// 新增实例句柄类型
typedef struct PowerMeterInstance PowerMeterInstance;

// 修改函数声明以支持多实例
PowerMeterInstance* PowerMeter_Init(PowerMeter_Init_Config_s *config);
void PowerMeter_Update(PowerMeterInstance *instance);
float PowerMeter_GetBusV(PowerMeterInstance *instance);

/* 电源全局数据结构体 */
typedef struct {
    float bus_voltage;  // 单位：mV
    float current;      // 单位：mA
    float power;        // 单位：W
} PowerData_t;

/* 修改函数声明 */
void PowerMeter_Init(PowerMeter_Init_Config_s *config);
void PowerMeter_Update(void); // 新增全局更新函数
const PowerData_t* PowerMeter_GetData(void); // 新增数据获取接口
